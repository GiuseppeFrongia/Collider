        ...
        if(fAddNoise && (fNoiseMean > 0.0)) {
            for(const auto& geomPair : fLayerGeom) {
                ...
                for(int n = 0; n < nNoiseHits; ++n) {
                    double noiseZ = rand->Uniform(-geom.length / 2.0, geom.length / 2.0);
                    double noisePhi = rand->Uniform(-TMath::Pi(), TMath::Pi());
                    
                    hitsByLayer[layerID].push_back({noiseZ, noisePhi, geom.radius});
                }
            }
        }
        ...
        for(auto it1 = hitsByLayer.begin(); it1 != hitsByLayer.end(); ++it1) {
            for(auto it2 = std::next(it1); it2 != hitsByLayer.end(); ++it2) {
                ...
                for(const auto& h1 : innerHits){
                    ...
                    double phiMin = h1.phi - fDeltaPhiMax;
                    double phiMax = h1.phi + fDeltaPhiMax;
