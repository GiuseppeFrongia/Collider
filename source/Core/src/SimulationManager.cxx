...
fNEvents(1000),
fPileUpMean(1.0),
fVertexDistr(nullptr),
fMomentumDistr(nullptr),
fEtaDistr(nullptr),
fPhiDistr(nullptr),
fMultDistr(nullptr) {
}
...
bool SimulationManager::Init(const std::string& inputFile) {
    ...
    fVertexDistr   = fRand->Fact3DJointDistr(parser.GetRawString("VertexDistr", "Gaussian 0 0.01 Gaussian 0 0.01 Gaussian 0 0.53"));
    fMomentumDistr = fRand->SetDistr(parser.GetRawString("MomentumDistr", "Fixed 1.0"));
    fEtaDistr      = fRand->SetDistr(parser.GetRawString("EtaDistr", "Uniform -2.0 2.0"));
    fPhiDistr      = fRand->SetDistr(parser.GetRawString("PhiDistr", "Uniform 0 2*pi"));
    fMultDistr     = fRand->SetDistrInt(parser.GetRawString("MultDistr", "Poisson 20"));
...
void SimulationManager::Run() {
    if(!fTree || !fRand || !fVertexDistr || !fMomentumDistr || !fEtaDistr || !fMultDistr) return;
    ...
    for(int i = 0; i < fNEvents; ++i){
        ...
        for(int v = 0; v < nVertices; ++v){
            int mult = fMultDistr->Sample();
            
            fVertexDistr->Sample(xVertex, yVertex, zVertex);
            fEvent->AddVertex(xVertex, yVertex, zVertex, mult);
            
            for(int p = 0; p < mult; ++p){
                double eta = fEtaDistr->Sample();
                double theta = 2.0 * std::atan(std::exp(-eta));
                double phi = fPhiDistr->Sample();
                double momentum = fMomentumDistr->Sample();
                ...
                for(auto layer : fLayers){
                    ...
                    // transport restituisce true se la particella interagisce con il layer
                    if(fTransport->Propagate(part, layer)){
                        
                        hitBuffer.Clear();
                        // layer restituisce true solo se genera una Hit
                        if(layer->Interact(part, hitBuffer)) { fEvent->AddHit(hitBuffer); }
                    } else { continue; }
                    layerID++;
                }
                trackID++;
